  ...
  private registerEventListeners() {
    ...
    if (events.length > 0) {
      log
        .scope("event")
        .info(
          `Registering ${events.length} event listeners for ${this.constructor.name}`
        );
    }
    ...
    events.forEach(event => {
      ...
      const handler = async (...args: any[]) => {
        try {
          ...
          log
            .scope("event")
            .error(`Error in event handler ${eventName}`, error);
          if (event.ipcChannel && event.sendToRenderer !== false) {
      ...
      if (event.interval) {
        ...
        log
          .scope("event")
          .info(
            `  - [${event.interval}ms] ${event.ipcChannel} -> ${event.method}`
          );
      } else {
