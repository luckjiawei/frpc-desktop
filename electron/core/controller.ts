  ...
  private registerIpcHandlers() {
    ...
    if (routes.length > 0) {
      ...
      routes.forEach((route) => {
        log.scope("ipc").info(`  - [${route.ipcType}] ${route.path} -> ${route.method}`);
      });
    ...
    routes.forEach((route: IpcRouteMetadata) => {
      ...
      if (route.ipcType === "on") {
        ipcMain.on(route.path, async (event: any, ...args: any[]) => {
          if (args) {
            scope.debug(`[Request] ${route.path} -> ${this.constructor.name}.${route.method} params => ${JSON.stringify(args)}`);
          } else {
            scope.debug(`[Request] ${route.path} -> ${this.constructor.name}.${route.method}`);
          }
          try {
            const result = await handler(event, ...args);
            if (!route.manualReply) {
              scope.debug(`[Response] ${route.path} result => ${JSON.stringify(result)}`);
        ...
        ipcMain.handle(route.path, async (event: any, ...args: any[]) => {
          ...
          try {
            const result = await handler(event, ...args);
            scope.info(`[Response] ${route.path} result => ${JSON.stringify(result)}`);
